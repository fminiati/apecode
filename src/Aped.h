...
namespace APED {
  ...
  template <class T> void add(vector<T>& va, const vector<T>& vb, const T f)
  {
  ...
  template <class T> T linear_interp(const T xp, const vector<T>& x, const vector<T>& f)
  {
    //
    if (xp<x.front() || xp>=x.back()) return T(0);
  ...
  // return interpolated function (f(x)) value at input position xp
  template <class T, class R> void linear_interp(vector<T>&       fp, const vector<T>& xp,
  ...
  // return interpolated function (f(x)) value at input position xp
  template <class T, class R> void linear_integrate(vector<T>&       fp, const vector<T>& xp,
                                                    const vector<R>& f,  const vector<R>& x)
  {
    //
    size_t ip=0;
...
//     GaussianKernel(const Real a_delta, const double a_tol)
//     {
//       // builds kernel with size sufficient to have negligible residuals in the wings
//       const double delta=(double)a_delta;
...
//         ++i;
//         w =erf(i*a_delta);
//         const double dw=w-(one-err);
//         m_w.push_back(half*dw);
//         m_w.insert(m_w.begin(),half*dw);
//         // update error
//         err=one-w;
//       }
//       assert(m_w.size()%2==1);
...
//     }
  ...
  static const float AndersGrevesseAbundances[NUM_APEC_ATOMS]={12.0,10.99,1.16,1.15,2.60,8.56,8.05,
    8.93,4.56,8.09,6.33,7.58,6.47,7.55,5.45,7.21,5.50,6.56,5.12,6.36,3.10,4.99,4.00,5.67,5.39,
    7.67,4.92,6.25,4.21,4.60};
  
  static const float LoddersAbundances[NUM_APEC_ATOMS]={12.0,10.984,3.35,1.48,2.85,8.46,7.90,
    8.76,4.53,7.95,6.37,7.62,6.54,7.61,5.54,7.26,5.33,6.62,5.18,6.41,3.15,5.00,4.07,5.72,5.58,
    7.54,4.98,6.29,4.34,4.70};
...
#define NBBNELEMENTS (4)
  ...
  struct AbundanceUtil {
                             ...
                             const string            a_abundances_model)
    {
      ...
      for (vector<unsigned>::const_iterator a=a_elements.begin(); a!=a_elements.end(); ++a)
        if (*a>NUM_APEC_ATOMS) throw std::runtime_error("AbundanceUtil:: atomic number is out of range ! ");
      
      // if elements.size()==0, use all elements
      vector<unsigned>* elements;
      if (a_elements.size()>0) elements=new vector<unsigned>(a_elements.begin(),a_elements.end());
      else                     elements=new vector<unsigned>(APED_atomic_numbers,
      ...
      if (a_abundances_model=="AndersGrevesse") {
        mod_ab=new vector<float>(AndersGrevesseAbundances,AndersGrevesseAbundances+NUM_APEC_ATOMS);
      }
      else if (a_abundances_model=="Lodders") {
        mod_ab=new vector<float>(LoddersAbundances,LoddersAbundances+NUM_APEC_ATOMS);
      }
      else {
  ...
  struct TemperatureRecord
  {
    ...
    ~TemperatureRecord() {}
    // copy constructor
    void operator=(const TemperatureRecord& a_rhs)
    {
  ...
  protected:
                      ...
                      const LineKernel* a_kernel) const
    {
      if (a_line_energy>=a_energy.front() && a_line_energy<a_energy.back())
      {
        ...
        int ie=0;
        while (a_energy[ie]<a_line_energy) ++ie;
        ie=std::max(0,ie-1);
