...
//             ßconst Real atomic_mass= AMU_g*el->second.atomic_mass; //m_aped_data[it].elements[A->atomic_number].atomic_mass;
                             ...
                             const string         a_line_broadening) const
{
  ...
  map<unsigned,Element>::const_iterator ei=m_aped_data[a_temp_idx].elements.find(a_atomic_number);
  if (ei != m_aped_data[a_temp_idx].elements.end())
  {
    ...
    map<unsigned,Ion>::const_iterator ii;
    if (a_rmJ==0) ii=atom.ions.begin();
    else          ii=atom.ions.find(a_rmJ);
    ...
    for ( ; ii!=atom.ions.end(); ++ii) {
      ...
      vector<float>::const_iterator j=ion.line_emissivity.begin();
      for (vector<float>::const_iterator hn=ion.line_energy.begin(); hn!=ion.line_energy.end(); ++hn, ++j) {
                                  ...
                                  const Real           a_doppler_shift) const
{
  ...
  if (ei != m_aped_data[a_temp_idx].elements.end())
  {
    ...
    // if ionization state (rmJ) == 0 then add up all ions, else select ionization state according to input
    map<unsigned,Ion>::const_iterator it=atom.ions.find(a_rmJ);
    if (it!=atom.ions.end())
    {
      // ion
      ...
      if (a_doppler_shift!=0.e0) {
        ...
        for (vector<float>::iterator vi=cnt_enrg_shifted.begin(); vi!=cnt_enrg_shifted.end(); ++vi) {
          *vi *= (one+a_doppler_shift);
        }
        for (vector<float>::iterator vi=psd_enrg_shifted.begin(); vi!=psd_enrg_shifted.end(); ++vi) {
          *vi *= (one+a_doppler_shift);
        }
      }
